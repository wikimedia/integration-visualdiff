...
VisualDiffer.takeScreenShot = async function(browser, logger, opts, htmlOpts) {
	...
	// Bypass CSP so we can inject JS and run it locally
	...
	page.on('error', function(msg, trace) {
		...
		if (!/leftTab.getBoundingClientRect|evaluating 'defaultUri/.test(msg)) {
			console.log("ERROR MSG: " + msg + "; TRACE: " + JSON.stringify(trace));
		}
	});
	...
	try {
		...
		htmlOpts.err = 'Could not open page ' + htmlOpts.url + '. Error: ' + JSON.stringify(error);
		return testCompletion(browser, opts);
	}

	await Promise.delay(opts.screenShotDelay * 1000);
	if (htmlOpts.injectJQuery) {
		// FIXME: This won't run because of CSP issues and so use of jquery
		// in the post-processing script will fail
