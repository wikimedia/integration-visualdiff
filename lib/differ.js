...
VisualDiffer._getBrowser = function(opts, logger, retryCount, cb) {
	...
	phantom.create('--debug=false', '--ssl-protocol=TLSv1', '--proxy=' + proxy, function (ph) {
		if (!ph) {
			if (retryCount < 10) {
				logger('Port conflict starting phantomjs. Retrying!');
				VisualDiffer._getBrowser(opts, logger, retryCount + 1, cb);
			} else {
				cb('Failed to set up phantomjs at a non-conflicting port!');
			}
			return;
		}
...
VisualDiffer.takeScreenshots = function(opts, logger, cb) {
	...
	var self = this;
	VisualDiffer._getBrowser(opts, logger, 0, function(err, browser) {
		if (err) {
			cb(err);
			return;
		}
...
VisualDiffer.compareWithUprightDiff = function(opts, logger, cb) {
	try {
			...
			function (error, stdout, stderr) {
				if (error && error.code !== 0) {
					...
					if (opts.jsonFormat) {
						...
						out.skips = Math.round(0.5 * 100 * data.modifiedArea / data.totalArea * 0.5 + 100 * data.movedArea / data.totalArea * 0.5);
						out.raw = stdout;
						cb(null, out);
					} else {
						cb(null, stdout);
		...
		// Catch exceptions and pass along so it is retried
		logger(e);
		cb(e);
	}
};
