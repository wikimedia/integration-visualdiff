	...
	async takeScreenshots(opts, logger) {
		...
		try {
			...
			if (opts.viewportHeight == 0) {
				...
				// setting viewport height to max + 20 to ensure that both images are the same height.
				...
				// 5% is a somewhat arbitrary threshold I picked here
				if ( variance / minHeight < 0.05 ) {
					// Looks like just setting max width doesn't eliminate diffs. So, add a 20px buffer.
					puppeteerOpts.defaultViewport.height = Math.max(html1Dims.height, html2Dims.height) + 20;
