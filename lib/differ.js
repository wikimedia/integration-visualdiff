			...
			return page.open(htmlOpts.url).then(function (status) {
				...
				return Promise.delay(opts.screenShotDelay * 1000).then(function() {
					if (htmlOpts.injectJQuery) {
						return page.includeJs('http://ajax.googleapis.com/ajax/libs/jquery/1.9.1/jquery.min.js').then(function() { return processPage() });
					} else {
						return processPage();
					}
				});
...
VisualDiffer.compareWithUprightDiff = function(opts, logger) {
	...
	return Promise.resolve().then(function() {
		...
		return uprightDiff(opts.uprightDiffPath || '/usr/local/bin/uprightdiff',
			[
				'--format=json',
