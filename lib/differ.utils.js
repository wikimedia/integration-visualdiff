...
const transformForMobileIfNeeded = ( url, opts ) => {
	const [ base, originalQuerystring ] = url.split( '?' );
	const querystring = originalQuerystring ? `${ originalQuerystring }&` : '';
	return opts.mobile ? `${ base }?${ querystring }useformat=mobile&useskin=minerva` : url;
};

const transformForVariantIfNeeded = ( url, opts ) => {
	const [ base, originalQuerystring ] = url.split( '?' );
	const querystring = originalQuerystring ? `${ originalQuerystring }&` : '';
	return opts.variant ? `${ base }?${ querystring }variant=${ opts.variant }` : url;
};

const transformIfNeeded = Util.transformIfNeeded = ( url, opts ) => {
	url = transformForMobileIfNeeded( url, opts );
	url = transformForVariantIfNeeded( url, opts );
	return url;
};
...
var computeOpts = function(argv, opts) {
	...
	// Support language variants: split the variant from the wiki name.
	// (wiki name will look like 'srwiki.sr-ec' for a language variant wiki)
	[opts.wiki, opts.variant] = opts.wiki.split('.', 2);
	...
	if (!opts.html1.url && opts.html1.computeURL) {
		opts.html1.url = transformIfNeeded(
			opts.html1.computeURL(opts.html1.server, opts.wiki, opts.title),
			opts
		);
	}
	...
	if (!opts.html2.url && opts.html2.computeURL) {
		opts.html2.url = transformIfNeeded(
			opts.html2.computeURL(opts.html2.server, opts.wiki, opts.title),
			opts
		);
	}
