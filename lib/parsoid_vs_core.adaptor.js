...
function generateLocalHTMLFiles(opts) {
	...
	.spread(function(_, res) {
		...
		.spread(function(_, res) {
			...
			// Transform <p>(style|link|span-with-single-discussion-tools-marker-comment|></p>
			...
			Array.from(dom.querySelectorAll('p')).map(function(p) {
				...
				while (c) {
					if (c.nodeName !== 'LINK' &&
						c.nodeName !== 'STYLE' &&
						// Ignore anything that is not a span with a single comment
						!(c.nodeName === 'SPAN' && (!c.firstChild || (c.firstChild === c.lastChild && c.firstChild.nodeType === 8)))
					) {
						empty = false;
						break;
					}
