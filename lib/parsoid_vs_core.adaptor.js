...
function generateLocalHTMLFiles(opts) {
	...
	.spread(function(_, body) {
		...
		// Add stylesheets
		addStyleSheet(dom, '/w/load.php?modules=skins.vector.styles.legacy|jquery.makeCollapsible.styles|jquery.tablesorter.styles&only=styles&skin=vector&useskinversion=1&lang=' + lang);
		...
		// addScriptTag(dom, { path: opts.scriptDir + '/jquery.js' });
		addScriptTag(dom, { content: 'document.documentElement.className="client-js";RLCONF={};RLSTATE={};RLPAGEMODULES=["site","jquery.tablesorter","jquery.makeCollapsible","skins.vector.legacy.js","mediawiki.page.ready","ext.kartographer.link","ext.kartographer.frame"];' });
